var timesheetLookup = (function() {
  ...
  return {
    ...
    setup_job_validation_callback1: function(job_number, job_name) {
        ...
        ajaxRequest.open("GET", "/cgi-bin/timesheet_lookup.pl?task=job_validation1&callback=timesheetLookup.job_validation_callback1&job=" + job_phase[0] + "&phase=1", true); // + job_phase[1], true);
	...
	if ( error == true ) {
	    ...
	    // A valid number
	    // If this job number already exists in the list, don't add it,
	    // just select it
	    var jobfound = false;
	    for ( var k=0; k<document.main.select_job.options.length; k++){
		if ( document.main.select_job.options[k].value == document.main.jobNumber.value) {
		    jobfound = true;
		    document.main.select_job.options[k].selected = true;
		    break;
		}
	     }
	     // If this job number doesn't exist, add it to the list
	     if ( jobfound == false ) {
		 var opt = new Option(document.main.jobNumber.value + " - " + job_name, document.main.jobNumber.value, true, true);
		 document.main.select_job.options[document.main.select_job.options.length] = opt;
	     }
	     // Get the activity list from MySQL
	     if ( document.main.jobNumber.value == '100.01' ) {
		 //timesheetLookup.service_activity1(document.main.activity.value);
	     // TODO: Was changed always true? } else if ( changed == 'true' ) {
	     } else {
		 timesheetLookup.activity_list1(document.main.jobNumber.value);
		 timesheetLookup.mark_list_data1(document.main.jobNumber.value);
		 timesheetLookup.subledger_list_data1(document.main.jobNumber.value.split(".")[0]);
	     }
	     badRecord = false;
	}
    },
	...
	document.main.select_act.options[0] = new Option("(Cost Code)", "Cost Code", true, true);
	
	timesheetLookup.known_jobs[job] = list_name;

        for ( var i = 0; i < result.length; i++ ) {
            document.main.select_act.options[i+1] = new Option(result[i][1] + " - " + result[i][0], result[i][1], false, false);
        }
        timesheetLookup.activityListLoaded = true;
    },
...
console.log('result: %s', result);
	...
	timesheetLookup.known_jobs[job] = list_name;
        ...
        for ( var i = 0; i < result.length; i++ ) {
            //preloaded_activity_lists[list_name][i] = new Array(result[i][0], result[i][1]);
    ...
    //
    // Start to get subledger list
    //
    
    subledger_list1: function(job) {

        timesheetLookup.setup_subledger_list_callback1(job);

    },

    //
    // Set up ajax validation to take place in subledger_list_callback1()
    //

    setup_subledger_list_callback1: function(job) {
        ...
        timesheetLookup.job = job;
        ajaxRequest.open("GET", "/cgi-bin/timesheet_lookup.pl?task=subledger_list1&callback=timesheetLookup.subledger_list_callback1&code=100&job=" + job, true);
        ajaxRequest.send(null);
   ...
   subledger_list_callback1: function(json) {
        var error = false;
        var line = json.data;
        // TODO Must be shared.
        // var known_jobs = new Array();
        ...
        timesheetLookup.mark_list_name = mark_list_name;
        if ( code=="200 " ) {
                timesheetLookup.subledger_list_data1(job, mark_list_name);
        } else {
             // Error
             return;
        }
    ...
    subledger_list_data1: function(job, list_name) {

        timesheetLookup.setup_subledger_list_data_callback1(job, list_name);
    },
    ...
    setup_subledger_list_data_callback1: function(job, list_name) {
        ...
        ajaxRequest.open("GET", "/cgi-bin/timesheet_lookup.pl?task=subledger_list1&callback=timesheetLookup.subledger_list_data_callback1&code=202&job=" + job,true);// + "&list=" + list_name, true);
        ajaxRequest.send(null);
    ...
    subledger_list_data_callback1: function(json) {
        var error = false;
        var line = json.data;
        var job = timesheetLookup.job;
        var list_name = timesheetLookup.mark_list_name;
        ...
        line = line.replace(/~~~/g, "\n");
        eval(line);
        document.main.select_subled.options.length = 0;
        document.main.select_subled.options[0] = new Option("(Order Number)", "Order Number", true, true);
        timesheetLookup.known_jobs[job] = list_name;

        for ( var i = 0; i < result.length; i++ ) {
            document.main.select_subled.options[document.main.select_subled.options.length] = new Option(result[i][1] + " - " + result[i][0], result[i][1], false, false);
        }
        timesheetLookup.activityListLoaded = true;
    },
