...
function ts_sort_date(a,b) {
    // y2k notes: two digit years less than 50 are treated as 20XX, greater than50 are treated as 19XX
    aa = ts_getInnerText(a.cells[SORT_COLUMN_INDEX]);
    bb = ts_getInnerText(b.cells[SORT_COLUMN_INDEX]);
    var yr;
    if (aa.length == 10) {
        ...
        yr = aa.substr(6,2);
        if (parseInt(yr) < 50) { yr = '20'+yr; } else { yr = '19'+yr; }
        dt1 = yr+aa.substr(0,2) + aa.substr(3,2) + aa.substr(9,2)
             + aa.substr(12,2);
    } else {
        yr = aa.substr(6,2);
        if (parseInt(yr) < 50) { yr = '20'+yr; } else { yr = '19'+yr; }
    ...
    } else if (bb.length == 14) {
        yr = bb.substr(6,2);
        if (parseInt(yr) < 50) { yr = '20'+yr; } else { yr = '19'+yr; }
        dt2 = yr+bb.substr(0,2)+bb.substr(3,2) + bb.substr(9,2)
             + bb.substr(12,2);
    } else {
        yr = bb.substr(6,2);
        if (parseInt(yr) < 50) { yr = '20'+yr; } else { yr = '19'+yr; }
...
function ts_get_cookie(table_id)
{
  var cookie="; "+document.cookie.toString()+"; "
	...
	if(start>-1)
	{
		st = cookie.indexOf("; ",start+searchString.length);
		tsCookie = cookie.substring(start+searchString.length,st);
...
function alternateRowColors2(table) {
