var timesheetLookup = (function() {
  ...
  return {
	...
	if ( error == true ) {
		...
		if ( document.main.select_job.options[k].value
		     == document.main.jobNumber.value+" - " + job_name) {
		    jobfound = true;
	...
	document.main.select_act.options[0] =
        ...
        TODO ?
	Hard code into list.
        foreach my $activity (@$perm_list) {
           if ( $activity eq 'JURY DUTY' ) {
                print <<"EOT";
                   document.main.select_act.options[document.main.select_act.options.length] = new Option("$activity", "JURY", false, false);
EOT
           } elsif ( $activity eq 'BEREAVEMENT' ) {     
                print <<"EOT";
                   document.main.select_act.options[document.main.select_act.options.length] = new Option("$activity", "BEREAVEM", false, false);
EOT
           } else {
                print <<"EOT";
                   document.main.select_act.options[document.main.select_act.options.length] = new Option("$activity", "$activity", false, false);
EOT
           }
        }
        */
...
console.log('result: %s', result);
   ...
   mark_list_callback1: function(json) {
 	...
 	code = line.substring(0, 4);
        ...
        if ( code=="200 " ) {
        /*          timesheetLookup.markListLoaded = true;
             if ( check_preloaded(mark_list_name) == true ) {
	          timesheetLookup.known_jobs[job] = mark_list_name;
                  timesheetLookup.markListLoaded = true;
                  return;
             }*/
        } else {
	...
	timesheetLookup.known_jobs[job] = list_name;
        preloaded_marks[list_name] = new Array();

        for ( var i = 0; i < result.length; i++ ) {
            preloaded_marks[list_name][i] =
                new Array(result[i][0], result[i][1]);
